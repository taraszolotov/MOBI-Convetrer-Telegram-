...
def convert_to_mobi(input_file: str, author: str, title: str) -> str:
    ...
    try:
        ...
        subprocess.run(['ebook-convert', input_file, output_file, '--authors', author, '--title', title], check=True, timeout=120)
        return output_file
        ...
        try:
            if input_file.endswith('.pdf'):
                ...
                subprocess.run(['ebook-convert', html_file, output_file, '--authors', author, '--title', title], check=True, timeout=120)
                return output_file
        except subprocess.CalledProcessError:
            return None
